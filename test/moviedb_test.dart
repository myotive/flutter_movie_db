...
void main(){
  ...
  test('Test MovieDB Upcoming', () async {
    var result = await movieDB.upcomingMovies();
  ...
  test('Test MovieDB Disovery With Criteria', () async {
    var result = await movieDB.discoverMovieWithCriteria();
    expect(result.results.length > 0, isTrue);
    expect(result.results[0], isNotNull);
    expect(result.page == 1, isTrue);

    var result2 = await movieDB.discoverMovieWithCriteria(page: 2);
    expect(result2.results.length > 0, isTrue);
    expect(result2.results[0], isNotNull);
    expect(result2.page == 2, isTrue);
  ...
  test('Test MovieDB Get Film By Id', () async {
    var result = await movieDB.getMovieById(332562);
